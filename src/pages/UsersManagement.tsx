...
import { Shield, Plus, Edit2, Trash2, Users, Check, X, Building2, UserCircle2 } from 'lucide-react';
              ...
              {!isClientModal && (
                <div>
                  <div className="flex items-center justify-between py-1">
                    ...
                    {!addingUnit && (
                      <button type="button" onClick={() => { setAddingUnit(true); setNewUnitName(''); setTimeout(() => newUnitInputRef.current?.focus(), 50); }} className="flex items-center gap-1 text-[10px] font-bold text-blue-600 hover:text-blue-700 uppercase tracking-wider px-2 py-1 rounded-lg hover:bg-blue-50 transition-all">
                  ...
                  {addingUnit && (
                    <div className="flex items-center gap-2 mb-2 p-2.5 bg-blue-50 border border-blue-200 rounded-xl">
                      <input ref={newUnitInputRef} type="text" placeholder="Unit name (e.g. Events)" value={newUnitName} onChange={(e) => setNewUnitName(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { e.preventDefault(); handleAddUnit(); } if (e.key === 'Escape') { setAddingUnit(false); setNewUnitName(''); } }} className="input input-bordered input-xs bg-white border-blue-200 rounded-lg flex-1 h-9 text-sm font-medium" />
                      <button type="button" onClick={handleAddUnit} disabled={addUnitLoading || !newUnitName.trim()} className="btn btn-xs bg-blue-600 hover:bg-blue-700 border-blue-600 text-white rounded-lg w-9 h-9 p-0 flex items-center justify-center">
                        {addUnitLoading ? <span className="loading loading-spinner loading-xs" /> : <Check className="w-3.5 h-3.5" />}
                      </button>
                      <button type="button" onClick={() => { setAddingUnit(false); setNewUnitName(''); }} className="btn btn-xs btn-ghost text-slate-500 hover:bg-slate-100 rounded-lg w-9 h-9 p-0 flex items-center justify-center">
